...
class GridDataConverter(nn.Module):
    ...
    Args:
        ...
        normalize_features (bool): If True normalizes features (i.e. RGB values)
            to lie in [-1, 1].
    """
    def __init__(self, device=None, data_shape=None, normalize=True,
                 normalize_features=False):
        super(GridDataConverter, self).__init__()
        self.device = device
        ...
        # Register coordinates as a buffer to ensure it moves with .to(device)
        self.register_buffer("coordinates", torch.ones(data_shape[1:]).nonzero(as_tuple=False).float())
...
class PointCloudDataConverter(nn.Module):
    ...
    """
    def __init__(self, data_shape, normalize=True,
                 normalize_features=False):
        super(PointCloudDataConverter, self).__init__()
        self.data_shape = data_shape
        self.normalize = normalize
        self.normalize_features = normalize_features
        self.coordinate_dim = len(data_shape[1:])
...
class ERA5Converter(nn.Module):
    ...
    Notes:
        ...
        The coordinates are given by:
            ...
            z = sin(latitude).
    """
    def __init__(self, data_shape, normalize=True, normalize_features=False):
        super(ERA5Converter, self).__init__()
        self.data_shape = data_shape
        self.normalize = normalize
        self.normalize_features = normalize_features
        ...
        self.longitude = np.linspace(0., 360. - (360. / data_shape[1]), data_shape[1])
        ...
        data_tensor = np.stack([latitude_grid, longitude_grid, np.zeros_like(longitude_grid)])
        data_tensor = torch.Tensor(data_tensor)

        # Shape (num_lats, num_lons, 3)
        coordinates, _ = era5_to_coordinates_and_features(data_tensor)
        ...
        longitude_grid = torch.Tensor(longitude_grid)
        self.register_buffer('latitude_grid', latitude_grid)
        self.register_buffer('longitude_grid', longitude_grid)
...
class ImageResize(nn.Module):
    def __init__(self, size):
        ...
        if isinstance(size, int):
            size = (size,size)
        self.downsample = transforms.Resize(size, interpolation=transforms.InterpolationMode.NEAREST)
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Downsampled image of shape (B, C, target_H, target_W)
        """
